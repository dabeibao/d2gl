...
"uniform sampler2DArray u_CursorTexture,u_FontTexture,u_ExternalTexture,u_ItemColorLUT,u_Texture;"
"layout(std140) uniform ubo_Colors{vec4 u_Palette[256];vec4 u_Gamma[256];};"
...
"uniform bool u_IsMasking=false,u_IsGlide=true,u_HasMask=false;"
...
"{"
  "if(v_Extra.x<0.)"
  "{"
    "vec4 G=v_Color1.yzwx;"
    "if(v_Flags.y==1u)"
      "FragColor=v_Color2;"
    "else "
      "{"
        "float t=texture(u_Texture,vec3(v_TexCoord,v_TexIds.x)).x;"
        "if(v_Flags.x==1u&&t==0.)"
          "discard;"
        "FragColor=G*u_Palette[int(t*255)];"
      "}"
    ...
    "FragColor.w=v_Flags.z==1u? v_Color2.w:1.;"
  "}"
  "else "
  "{"
  "switch(v_Flags.x){"
    ...
    "{"
      ...
      "if(tc.a<.01){FragColor=tc*v_Color1;break;}"
      "vec3 src=tc.rgb;"
      "if(v_Extra.y>.5&&int(v_TexIds.y)>0)"
      "{"
        "float ri=src.r*31.,gi=src.g*31.,bi=src.b*31.;"
        "float g=round(gi);"
        "vec2 uv=vec2((ri+.5+g*32.)/1024.,(bi+.5)/32.);"
  ...
  "if(v_Flags.x!=3u&&v_Flags.x!=8u)"
    "{"
      ...
      "if(v_Flags.z>0u)"
        "FragColor.w*=float(v_Flags.z)*.01;"
    "}"
  "if(u_IsGlide&&u_HasMask&&texture(u_MaskTexture,(gl_FragCoord.xy-u_Viewport.xy)/u_Viewport.zw).x>.1)"
    "FragColor.w=0.;"
  "}"
"}"
"\n#endif"
